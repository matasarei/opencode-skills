...
Commands run on the host system (no container). Ensure you have `gh`, `node` (the guard suite loads the plugin), `shellcheck`, and an OpenCode model server at 64k or 128k; Docker is not needed here.
