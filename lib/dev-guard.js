...
// Local models frequently pass a file path (e.g. "application/lib/CompanyEditForm.php") instead of a
// directory to glob("*.php"), causing OpenCode to crash with "glob path must be a directory: ...".
// If the target path is a file (or has a file extension whose parent exists), resolve to its parent directory.
export function sanitizeGlobPath(targetPath) {
...
export function sanitizeBashTimeout(timeout) {
...
export function checkConsecutiveCalls(tool, args) {
...
export const DevGuard = async ({ directory }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // 1. Sanitize grep regex patterns to prevent fatal ripgrep crashes on PHP namespaces
      ...
      // 2. Prevent infinite edit retry loops on already-updated or mismatched files
