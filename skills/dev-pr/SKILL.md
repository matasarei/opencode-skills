...
- `red-planned:` names a test and `red:` is not that test's `RED PROVEN` — red first never happened, and cannot be proven once the code exists. Say what it says and stop.
...
5. **Testing** — the `tested:` verdict, and `red:` when not `none`, quoted. When it names a commit rather than this HEAD, say which. Never a green line nothing produced.
...
Always `--draft`. `pr:` already open → `gh pr edit <number> --body-file .devskills/pr-body.md`, **only** when its body still opens with your `Depends on` or `Step <n>` line; a body somebody edited by hand → show yours and ask. A hand-written title stays.
